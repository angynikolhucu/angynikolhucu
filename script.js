...
function adjustElementsForTheme(theme) {
    ...
    // Crear nubes solo en modo claro
    if (theme === 'light') {
        createClouds();
    }
}
...
function createClouds() {
    ...
    // Crear más nubes para mejor efecto
    const cloudCount = 12;
    const sizes = ['small', 'medium', 'large'];
    ...
    for (let i = 0; i < cloudCount; i++) {
        ...
        const topPosition = Math.random() * 80 + 5; // Entre 5% y 85%
        cloud.style.top = `${topPosition}%`;
        
        // Velocidad de animación según tamaño (más grandes más lentas)
        let duration;
        if (randomSize === 'small') {
            duration = 25 + Math.random() * 20; // 25-45s
        } else if (randomSize === 'medium') {
            duration = 40 + Math.random() * 30; // 40-70s
        } else {
            duration = 60 + Math.random() * 40; // 60-100s
        }
        ...
        // Altura de capa (z-index) para efecto de profundidad
        if (randomSize === 'small') {
